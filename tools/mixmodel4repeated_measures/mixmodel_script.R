...
lmRepeated2FF <- function(ids, ifixfact, itime, isubject, ivd, ndim, nameVar = colnames(ids)[[ivd]],
    ...
    if (skip == 0) {
        ...
        res[1, ] <- rShapiro$p.value
        rownames(res)[1] <- "Shapiro.pvalue.residuals"
        res[2, ] <- rsum$varcor$subject[1]
        rownames(res)[2] <- "Subject.Variance"
        res[3, ] <- rsum$devcomp$cmp[7]
        rownames(res)[3] <- "REML"
        ...
        if (length(which(raov[, 6] <= pvalCutof)) > 0 & visu == "yes") {
            diagmflF(mfl,
                title = tit, pvalCutof = pvalCutof, least.confounded = least.confounded,
...
lmixedm <- function(datMN,
    ...
    if (fixfact == "none") {
        ...
        ifixfact <- which(colnames(sampids) == fixfact)
    }
    ...
    for (i in firstvar:lastvar) {
        ...
        tryCatch(
            {
                if (ifixfact > 0) {
                    reslmer <- lmRepeated2FF(subds,
                        ifixfact = 1, itime = 2, isubject = 3, ivd = 4, ndim = ndim, visu = visu,
                        tit = colnames(dslm)[i], pvalCutof = pvalCutof,
                        dffOption = dffOption, least.confounded = least.confounded,
                        outlier.limit = outlier.limit
                    )
                } else {
                    reslmer <- lmRepeated1FF(subds,
                        ifixfact = 0, itime = 1, isubject = 2, ivd = 3, ndim = ndim,
