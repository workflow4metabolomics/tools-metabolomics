...
diagmflF <- function(mfl,
    ...
    grid.arrange(blank,
        plot_linearity(diagLs,
            hlimitN = outlier.limit, plotL = FALSE,
            label_factor = c(uniC, fixC, timC)
        ),
        blank,
        plot_conditionalResiduals(diagLs,
            hlimitN = outlier.limit, plotL = FALSE,
            label_factor = c(uniC, fixC, timC)
        ),
        blank,
...
plot_mahalanobis <- function(diagLs, plotL = TRUE) {
    unitDf <- data.frame(
        unit = names(diagLs$std.mahalanobis.distance),
...
plot_condresQQplot <- function(diagLs, plotL = TRUE) {
    ...
    p <- qqplotF(
        x = df$standardized.conditional.residuals,
        distribution = "norm",
        line.estimate = NULL,
        conf = 0.95
    ) +
...
plot_lesaffreVeerbeke <- function(diagLs, plotL = TRUE) {
    unitDf <- data.frame(
        unit = names(diagLs$std.lesaffreverbeke.measure),
...
plot.res.Lmixed <- function(mfl, df, title = "", pvalCutof = 0.05) {
    ...
    rawPlot <-
        ...
        geom_point() +
        geom_line() +
        ggtitle("Individual time-courses (raw data)") +
        ylab(nameVar) +
    ...
    bPlot <-
        ...
        ggtitle(paste("Boxplot by ", fflab, sep = "")) +
        xlab("") +
        ylab("") +
    ...
    if (pvalCutof > bm) {
        bs <- pvalCutof
    } else if (pvalCutof < bm & pvalCutof > bi) {
        bm <- pvalCutof
        bs <- pvalCutof
    } else if (pvalCutof < bi) {
        bi <- pvalCutof
        bm <- pvalCutof
        bs <- pvalCutof
    }
