...
ReadFid <- function(path) {
  ...
  title <- str_split(readLines(paramFile)[1], ",", simplify = TRUE)[1, 2]
  version  <- as.numeric(substr(str_split(title, " ", simplify = TRUE)[1, 3], 1, 1))
  neo <- (version > 3)
...
checkArg <- function(arg, checks, can.be.null=FALSE) {
  check.list <- list(bool=c(is.logical, "a boolean"), int =c(function(x){x%%1 == 0}, "an integer"), num =c(is.numeric, "a numeric"), str =c(is.character, "a string"), pos =c(function(x){x > 0}, "positive"), pos0=c(function(x){x >= 0}, "positive or zero"), l1 =c(function(x){length(x) == 1}, "of length 1")
  )
  if (is.null(arg)) {
    if (!can.be.null) {
