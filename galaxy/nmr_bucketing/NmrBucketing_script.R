...
################################################################################################
NmrBucketing <- function(fileType, fileName, leftBorder = 10.0, rightBorder = 0.5, bucketSize = 0.04, exclusionZones, exclusionZonesBorders = NULL, graph = c("None", "Overlay", "One_per_individual"), nomFichier, savLog.txtC = NULL) {
  ## Option
  ##-------
  ...
  NmRBrucker_read <- function(DataDir, SampleSpectrum) {
    ...
    ACQFILE <- paste(DataDir, "/acqus", sep = "")
    SPECFILE <- file.path(DataDir, "pdata/1/1r", sep = "")
    PROCFILE <- file.path(DataDir, "pdata/1/procs", sep = "")
  ...
  while (j < dim(spectrum)[1]) {
    ...
    if (!is.null(exclusionZonesBorders)) {
      for (k in seq_len(nrow(exclusion.zone.m)))
  ...
  if (fileType == "zip") {
    ...
    while (i <= n) {
      ...
      orderedSpectrum <- rawSpectrum[order(rawSpectrum[,1], decreasing = TRUE), ]
  ...
  ## Inputs from dataset (already preprocessed files)
  if (fileType == "tsv") {
    FileNames <- colnames(fileName)
    n <- length(FileNames)
    
    for (i in seq_len(ncol(fileName))) {
      orderedSpectrum <- cbind(as.numeric(rownames(fileName)), fileName[,i])
      orderedSpectrum <- orderedSpectrum[order(orderedSpectrum[,1], decreasing = TRUE), ]
