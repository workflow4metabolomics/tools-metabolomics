...
table_match <- function(dataMatrix, sampleMetadata, variableMetadata) {
    ...
    if (length(missing_in_sm) > 0 || length(missing_in_dm) > 0) {
        ...
        if (length(missing_in_sm) > 0) {
            ...
            err.stock <- c(err.stock, prefix, "following identifiers found in the data matrix\n",
        ...
        if (length(missing_in_dm) > 0) {
            ...
            err.stock <- c(err.stock, prefix, "following identifiers found in the sample metadata file\n",
    ...
    if (length(missing_in_vm) > 0 || length(missing_in_dm_var) > 0) {
        ...
        if (length(missing_in_dm_var) > 0) {
            ...
            err.stock <- c(err.stock, prefix, "following identifiers found in the variable metadata file\n",
                           "    do not appear in the data matrix:\n",
...
intens_check <- function(DM.name, SM.name, VM.name, method, chosen.stat, class.col, test.fold, class1, fold.frac, 
  ...
  if (method == "no_class") {
    ...
    if (!(class.col %in% colnames(SM))) stop("The column ", class.col, " is not in sample metadata")
    c_class <- as.factor(SM[,class.col])
    nb_class <- nlevels(c_class)
    classnames <- levels(c_class)
    if (nb_class < 2 && test.fold == "Yes") cat("The column", class.col, "contains only one class, fold calculation could not be executed.\n")
    if (nb_class > (nrow(SM))/3 && method == "each_class") cat("There are too many classes, consider reducing the number.\n")
    if (method == "one_class") {
      if (!(class1 %in% classnames)) stop("The class ", class1, " does not appear in the column ", class.col)
      c_class <- factor(ifelse(c_class == class1, class1, "Other"), levels=c(class1, "Other"))
      nb_class <- nlevels(c_class)
      classnames <- levels(c_class)
    }
  }
  ...
  if (!is.numeric(as.matrix(DM))) {
    findchar <- function(myval) ifelse(is.na(myval), "ok", ifelse(is.na(as.numeric(as.character(myval))), "char", "ok"))
  ...
  for (j in 1:nb_class) {
    ...
    if ("mean" %in% stat.list) {
      ...
      if (j == nb_class) { stat.res <- cbind(stat.res, mean.res); names <- cbind(names, mean.names) }
    }
    # SD
    if ("sd" %in% stat.list) {
      sd.res <- cbind(sd.res, apply(DM[idx,-1],2,sd,na.rm=TRUE))
      sd.names <- cbind(sd.names, paste("Sd",classnames[j], sep="_"))
      if (j == nb_class) { stat.res <- cbind(stat.res, sd.res); names <- cbind(names, sd.names) }
    }
    # Median
    if (("median" %in% stat.list)&&(!("quartile" %in% stat.list))) {
      ...
      if (j == nb_class) { stat.res <- cbind(stat.res, med.res); names <- cbind(names, med.names) }
    }
    # Quartiles
    if ("quartile" %in% stat.list) {
      quart.res <- cbind(quart.res, t(apply(DM[idx,-1],2,quantile,na.rm=TRUE)))
      ...
      if (j == nb_class) { stat.res <- cbind(stat.res, quart.res); names <- cbind(names, quart.names) }
    }
    # Deciles
    if ("decile" %in% stat.list) {
      dec.res <- cbind(dec.res, t(apply(DM[idx,-1],2,quantile,na.rm=TRUE,seq(0,1,0.1))))
      dec.names <- cbind(dec.names, t(matrix(paste("D",seq(0,10,1),sep="_",classnames[j]))))
      if (j == nb_class) { stat.res <- cbind(stat.res, dec.res); names <- cbind(names, dec.names) }
    }
    # Missing values
    if ("NA" %in% stat.list) {
      ...
      if (j == nb_class) { stat.res <- cbind(stat.res, NA.res,pct_NA.res); names <- cbind(names, NA.names,pct_NA.names) }
      # Barplot data
    ...
    if (test.fold=="Yes" && nb_class >= 2 && j!=nb_class) {
      ...
      for (k in (j+1):nb_class) {
        ...
        if (logarithm=="log2") fold <- log2(fold)
        else if (logarithm=="log10") fold <- log10(fold)
        fold.res <- cbind(fold.res, fold)
        fname <- if (logarithm == "none") paste("fold",ratio1,"VS", ratio2, sep="_")
                 else paste(logarithm, "fold", ratio1, "VS", ratio2, sep="_")
        fold.names <- cbind(fold.names, fname)
      }
      if (j == nb_class) { stat.res <- cbind(stat.res,fold.res); names <- cbind(names, fold.names) }
    }
  }
...
parser$add_argument('--dataMatrix_in', required=TRUE, help='Input data matrix file')
parser$add_argument('--sampleMetadata_in', required=TRUE, help='Input sample metadata file')
parser$add_argument('--variableMetadata_in', required=TRUE, help='Input variable metadata file')
parser$add_argument('--method', required=TRUE, help='Computation method')
parser$add_argument('--chosen_stat', required=TRUE, help='Statistics to compute')
parser$add_argument('--class_col', default=NULL, help='Class column (optional)')
parser$add_argument('--test_fold', default=NULL, help='Test fold (optional)')
parser$add_argument('--class1', default=NULL, help='Class1 (optional)')
parser$add_argument('--fold_frac', default=NULL, help='Fold fraction (optional)')
parser$add_argument('--logarithm', default=NULL, help='Logarithm (optional)')
parser$add_argument('--variableMetadata_out', required=TRUE, help='Output variable metadata file')
parser$add_argument('--graphs_out', required=TRUE, help='Output graphs file')
