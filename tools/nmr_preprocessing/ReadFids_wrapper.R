...
# ------------------------------
option_list <- list(
  make_option(c("-f", "--fidzipfile"), type = "character",
              help = "Path to zipped Bruker FID file"),
  make_option(c("-t", "--title_line"), type = "character",
              help = "Title line for output"),
  make_option(c("-s", "--subdirectories"), action = "store_true",
              dest = "subdirectories", default = FALSE,
              help = "Whether to use subdirectories (boolean flag)"),
  make_option(c("-d", "--dirs_names"), action = "store_true",
...
# Data matrix
write.table(data_matrix, file = datamatrix, quote = FALSE, row.names = TRUE,
            sep = "\t", col.names = TRUE)

# Sample metadata
write.table(data_sample, file = samplemetadata, quote = FALSE, row.names = TRUE,
            sep = "\t", col.names = TRUE)
