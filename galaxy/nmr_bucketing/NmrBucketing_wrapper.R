...
## Computation
##------------
outputs <- NmrBucketing(fileType = fileType, fileName = directory, leftBorder = leftBorder, rightBorder = rightBorder, bucketSize = bucketSize, exclusionZones = exclusionZones, exclusionZonesBorders = exclusionZonesBorders, graph = graphique, nomFichier = nomGraphe, savLog.txtC = logFile)
...
if (graphique != "None") {
  excludedZone <- NULL
  for (c in seq_len(length(exclusionZonesBorders))) {
  ...
  if (graphique == "Overlay") {
    ...
    for (i in 1:ncol(data_bucket)) {
      ...
      for (g in 1:graphical.zone.length) {
        ...
        names(spectra) <- rownames(data_bucket)
        if (g == 1) {
          plot(1:length(spectra[1:(which(ppm == round(exclusionZonesBorders[[g]][1], 1))[1])]), spectra[1:(which(ppm == round(exclusionZonesBorders[[g]][1], 1))[1])], type = 'l', xlab = "", ylab = "Intensity", main = "", xaxt = "n")
          xPos <- 1
          nAxisPos <- 4
          startP <- length(nAxisPos)
          ...
          plot(1:length(spectra[(which(ppm == round(exclusionZonesBorders[[g - 1]][2], 1))[1]):(which(ppm == round(exclusionZonesBorders[[g]][1], 1))[1])]), spectra[(which(ppm == round(exclusionZonesBorders[[g - 1]][2], 1))[1]):(which(ppm == round(exclusionZonesBorders[[g]][1], 1))[1])], type = 'l', xlab = "", ylab = "Intensity", main = "", xaxt = "n")
          xPos <- 1
          nAxisPos <- 4
          startP <- length(nAxisPos) 
      ...
      plot(1:length(spectra[(which(ppm == round(exclusionZonesBorders[[g]][2], 1))[1]):nrow(data_bucket)]), spectra[(which(ppm == round(exclusionZonesBorders[[g]][2], 1))[1]):nrow(data_bucket)], type = 'l', xlab = "", ylab = "Intensity", main = "", xaxt = "n")
      xPos <- 1
      nAxisPos <- 4
      startP <- length(nAxisPos) 
