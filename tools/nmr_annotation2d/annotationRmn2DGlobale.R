...
# seuil : valeur du score de presence en de�a de laquelle les metabolites annotes ne sont pas retenus                                     #
...
annotationRmn2DGlobale <- function(template, tolPpm1 = 0.01, tolPpm2HJRes = 0.002, tolPpm2C = 0.5, cosy = 1, hmbc = 1, hsqc = 1, jres = 1, tocsy = 1,
    ...
    if (cosy == 1) {
        ...
        annotationCOSY <- annotationRmn2D(matrice.cosy, BdDReference_COSY, "COSY",
            ppm1Tol = tolPpm1, ppm2Tol = tolPpm1, seuil = seuilPls2D,
            unicite = unicite
        )
    ...
    if (hmbc == 1) {
        ...
        annotationHMBC <- annotationRmn2D(matrice.hmbc, BdDReference_HMBC, "HMBC",
            ppm1Tol = tolPpm1, ppm2Tol = tolPpm2C, seuil = seuilPls2D,
            unicite = unicite
        )
    ...
    if (hsqc == 1) {
        ...
        annotationHSQC <- annotationRmn2D(matrice.hsqc, BdDReference_HSQC, "HSQC",
            ppm1Tol = tolPpm1, ppm2Tol = tolPpm2C, seuil = seuilPls2D,
            unicite = unicite
        )
    ...
    if (jres == 1) {
        ...
        annotationJRES <- annotationRmn2D(matrice.jres, BdDReference_JRES, "JRES",
            ppm1Tol = tolPpm1, ppm2Tol = tolPpm2HJRes, seuil = seuilPls2D,
            unicite = unicite
        )
    ...
    if (tocsy == 1) {
        ...
        annotationTOCSY <- annotationRmn2D(matrice.tocsy, BdDReference_TOCSY, "TOCSY",
            ppm1Tol = tolPpm1, ppm2Tol = tolPpm1, seuil = seuilPls2D,
            unicite = unicite
        )
    ...
    return(list(
        COSY = annotationCOSY, HMBC = annotationHMBC, HSQC = annotationHSQC, JRES = annotationJRES, TOCSY = annotationTOCSY,
