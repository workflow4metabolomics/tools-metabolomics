...
begin_treatment <- function(name, signal_data = NULL, signal_info = NULL,
  ...
  return(list(
    start = proc.time(), vec = vec, force_real = force_real,
...
end_treatment <- function(name, begin_info, signal_data) {
  ...
  cat("End", name, "\n")
  cat(
    "It lasted", round(delta["user.self"], 3),
...
get_arg <- function(arg, info, argname, can_be_absent = FALSE) {
  if (is.null(arg)) {
    start <- paste(
      "impossible to get argument", argname,
...
group_delay_correction <- function(fid_data, fid_info = NULL,
  ...
  if (is.null(group_delay)) {
    ...
    group_delay_matrix <- matrix(
      c(
        44.75, 46, 46.311, 33.5, 36.5, 36.53, 66.625, 48, 47.87, 59.0833,
    ...
    if (!(toString(decim) %in% rownames(group_delay_matrix))) {
      stop(paste(
        "Invalid DECIM", decim, "it should be one of",
        rownames(group_delay_matrix)
      ))
    }
    if (!(toString(dspfvs) %in% colnames(group_delay_matrix))) {
      stop(paste(
        "Invalid DSPFVS", dspfvs, "it should be one of",
...
solvent_suppression <- function(fid_data, lambda_ss = 1e+06, ptw_ss = TRUE,
  ...
  if (return_solvent) {
    return(list(
      fid_data = fid_data, solvent_re = solvent_re,
...
fftshift_1d_2d <- function(x) {
  ...
  y <- x[, c((ceiling(dim(x)[2] / 2) + 1):dim(x)[2],
             1:ceiling(dim(x)[2] / 2)), drop = vec
  ]
  y
}
...
internal_referencing <- function(spectrum_data, fid_info,
                                 ...
                                 rowindex_graph = NULL) {
  # Data initialisation and checks
  begin_info <- begin_treatment("InternalReferencing",
                                spectrum_data, fid_info)
  ...
  if (range == "all") {
    ...
    if (n > 1) {
      data <- apply(
        Re(spectrum_data[, unlist(interval)]), 1,
...
zero_order_phase_correction <- function(spectrum_data,
  ...
  if (type_zopc %in% c("max", "rms")) {
    ...
    if (is.null(exclude_zopc)) {
      ...
      diff <- diff(unlist(exclude_zopc))[seq_along(diff(
                                                        unlist(exclude_zopc)))
      %% 2 != 0]
    ...
    for (k in 1:n) {
      ...
      if (debug_plot) {
        ...
        graphics::text(
          x = (ang + 0.1 * ang), y = (y[ang] - 0.1 * y[ang]),
          labels = round(ang, 3)
        )
      }

      # spectrum rotation
      spectrum_data[k, ] <- spectrum_data[k, ] * exp(complex(
        real = 0,
        imaginary = ang
      ))
      angles <- c(angles, ang)
    ...
    if (length(angle) != n) {
      stop(paste(
        "angle has length", length(angle), "and there are", n,
        "spectra to rotate."
      ))
    }
    for (k in 1:n) {
    ...
    for (k in 1:nn) {
      ...
      while (i <= n) {
        ...
        graphics::plot(Re(spectrum_data[i, ]),
          ...
          xlab = "Index", main = paste0(
            rownames(spectrum_data)[i],
            " - Real part"
          )
        )
...
baseline_correction <- function(spectrum_data, ptw_bc = TRUE, max_iter = 42,
  ...
  # Baseline estimation
  baseline <- matrix(NA, nrow = nrow(spectrum_data),
                     ncol = ncol(spectrum_data))
