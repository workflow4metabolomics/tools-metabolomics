univariateF <- function(datMN,
    ...
    if (tesC %in% c("ttest", "wilcoxon", "pearson", "spearman")) {
        ...
        nemMN <- PMCMRplus::kwAllPairsNemenyiTest(datMN[, 1], facFcVn, dist = "Tukey")[["p.value"]]
        nemVl <- c(lower.tri(nemMN, diag = TRUE))
        nemClaMC <- cbind(
            rownames(nemMN)[c(row(nemMN))][nemVl],
        ...
        nemMN <- t(apply(datMN, 2, function(varVn) {
            ...
            varNemMN <- PMCMRplus::kwAllPairsNemenyiTest(varVn, facFcVn, dist = "Tukey")[["p.value"]]
            c(pvaN, c(varNemMN))
        }))
