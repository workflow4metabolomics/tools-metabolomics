...
ok_norm <- function(qcp, qci, spl, spi, method, normref = NA, valimp = "0") {
    ...
    if (length(qcp[!is.na(qcp)]) < minQC) {
        ok <- 2
    } else {
        if (length(spl[!is.na(spl)]) < 2) {
            ok <- 2.5
        } else {
            if (sd(qcp, na.rm = TRUE) == 0 | sd(spl, na.rm = TRUE) == 0) {
                ok <- 1
            } else {
                cvp <- sd(qcp, na.rm = TRUE) / mean(qcp, na.rm = TRUE)
                cvs <- sd(spl, na.rm = TRUE) / mean(spl, na.rm = TRUE)
                ...
                if (rttest$p.value < 0.01) {
                    ok <- 3
                } else {
                    # to avoid denominator =0 when on 1 pool per batch <> 0
                    if (method == "linear" & length(which(((penteB * qci) + ordori) == 0)) > 0) {
                        ok <- 6
                    } else {
                        # different sloop between samples and pools
                        if (method == "linear" & penteB / penteS < -0.20) {
                            ok <- 5
                        } else {
                            #
                            if (method == "linear" & !is.na(normref) & valimp == "NA") {
                                ...
                                if (length(which((normval == Inf) | (denom < 1))) == length(normval)) {
                                    ok <- 6
                                }
                            }
                        }
...
plotsituation <- function(x, nbid, outfic = "plot_regression.pdf", outres = "PreNormSummary.txt", fact = "batch", span = "none",
                          sm_meta = list(
                              batch = "batch", injectionOrder = "injectionOrder", sampleType = "sampleType",
    ...
    for (p in 1:nbi) { # for each ion
        ...
        for (b in 1:nbb) { # for each batch...
            ...
            if (length(indpb) > 1) {
                ...
                if (length(which(!(is.na(xb[indsp, 3])))) > 1) {
                ...
                liminf <- min(xb[, 3], na.rm = TRUE)
                limsup <- max(xb[, 3], na.rm = TRUE)
                firstinj <- min(xb[, 2], na.rm = TRUE)
                lastinj <- max(xb[, 2], na.rm = TRUE)
                ...
                if (nrow(xb) > (length(indpb) + length(indsp))) {
                    points(xb[-c(indpb, indsp), 2], xb[-c(indpb, indsp), 3], pch = 18, col = "grey")
                }
                ...
                if (length(which(!(is.na(xb[indsp, 3])))) > 1) {
                    points(reslowessSample, type = "l", col = "red", lty = 2)
                }
        ...
        minval <- min(x[p + nbid], na.rm = TRUE)
        maxval <- max(x[p + nbid], na.rm = TRUE)
        plot(x[[sm_meta$injectionOrder]], x[, p + nbid],
            col = x[[sm_meta$batch]], ylim = c(minval, maxval), ylab = labion,
...
normlowess <- function(xb, detail = "no", vref = 1, b, span = NULL, valneg = "none", sm_meta = list(
                           batch = "batch", injectionOrder = "injectionOrder", sampleType = "sampleType",
                           sampleTag = list(pool = "pool", blank = "blank", sample = "sample")
                       ), min_norm = 1) {
    ...
    if (normTodo == 0) {
        if (length(span) == 0) {
            span2 <- 2 * length(indpb) / length(indsp)
        } else {
            span2 <- span
        }
        ...
        if (length(which(reslowess$y < min_norm)) != 0) { # to handle cases where 0<denominator<min_norm or negative
            ...
            if (valneg == "NA") {
                reslowess$y[toajust] <- NA
            } else {
                if (valneg == "0") {
                    reslowess$y[toajust] <- -1
                } else {
        ...
        if (detail == "reg") {
            ...
            plot(xb[indsp, 2], xb[indsp, 3], pch = 16, main = paste(labion, "batch ", b), ylab = "intensity", xlab = "injection order", ylim = c(liminf, limsup), xlim = c(firstinj, lastinj))
            if (nrow(xb) > (length(indpb) + length(indsp))) {
                points(xb[-c(indpb, indsp), 2], xb[-c(indpb, indsp), 3], pch = 18)
            }
            points(xb[indpb, 2], xb[indpb, 3], pch = 5)
            points(reslowess, type = "l", col = "red")
        }
...
normlinear <- function(xb, detail = "no", vref = 1, b, valneg = "none", sm_meta = list(
                           batch = "batch", injectionOrder = "injectionOrder", sampleType = "sampleType",
                           sampleTag = list(pool = "pool", blank = "blank", sample = "sample")
                       ), min_norm = 1) {
    # Correction function applied to 1 ion in 1 batch.
...
normloess <- function(xb, detail = "no", vref = 1, b, span = NULL, valneg = "none", sm_meta = list(
                          batch = "batch", injectionOrder = "injectionOrder", sampleType = "sampleType",
                          sampleTag = list(pool = "pool", blank = "blank", sample = "sample")
                      ), min_norm = 1) {
    # Correction function applied to 1 ion in 1 batch.
...
norm_QCpool <- function(x, nbid, outlog, fact, metaion, detail = "no", NormMoyPool = FALSE, NormInt = FALSE, method = "linear", span = "none", valNull = "0",
    ...
    if (detail == "plot") {
        if (nbb < 6) {
            par(mfrow = c(3, 3), ask = F, cex = 1.5)
        } else {
            par(mfrow = c(4, 4), ask = F, cex = 1.5)
        }
    }
    ...
    for (p in 1:nbi) { # for each ion
        ...
        if (length(which(pools1[!(is.na(pools1))] > 0)) < 2) { # if not enough pools >0 -> no normalisation
            ...
            if (detail == "reg" || detail == "plot") {
                ...
                maxval <- max(x[p + nbid], na.rm = TRUE)
                plot(x[[sm_meta$injectionOrder]], x[, p + nbid],
                    col = x[[sm_meta$batch]], ylab = labion, ylim = c(minval, maxval),
            ...
            for (b in 1:nbb) { # for every batch
                ...
                if (method == "linear") {
                    res.norm <- normlinear(sub, detail, valref[p], b, valNull, sm_meta, min_norm)
                } else {
                    if (method == "loess") {
                        res.norm <- normloess(sub, detail, valref[p], b, span, valNull, sm_meta, min_norm)
                    } else {
                        if (method == "lowess") {
                            res.norm <- normlowess(sub, detail, valref[p], b, span, valNull, sm_meta, min_norm)
                        } else {
            ...
            if (detail == "reg" || detail == "plot") {
                ...
                minval <- min(cbind(x[p + nbid], Xn[p + nbid]), na.rm = TRUE)
                maxval <- max(cbind(x[p + nbid], Xn[p + nbid]), na.rm = TRUE)
                plot(x[[sm_meta$injectionOrder]], x[, p + nbid],
                    col = x[[sm_meta$batch]], ylab = labion, ylim = c(minval, maxval),
                    main = paste0("before correction (CV for pools = ", round(cv[p, 1], 2), ")"), xlab = "injection order"
                )
                points(x[[sm_meta$injectionOrder]][indpool], x[indpool, p + nbid], col = "maroon", pch = 16, cex = 1)
                plot(Xn[[sm_meta$injectionOrder]], Xn[, p + nbid],
                    col = x[[sm_meta$batch]], ylab = "", ylim = c(minval, maxval),
    ...
    if (detail == "reg" || detail == "plot" || detail == "no") {
        if (nbi > 3) {
            ...
            plot(x[[sm_meta$injectionOrder]], xsum,
                col = x[[sm_meta$batch]], ylab = "sum of variables' intensities", xlab = "injection order",
            ...
            plot(x[[sm_meta$injectionOrder]], Xnsum,
                col = x[[sm_meta$batch]], ylab = "sum of variables' intensities", xlab = "injection order",
                ylim = c(min(c(xsum, Xnsum), na.rm = TRUE), max(c(xsum, Xnsum), na.rm = TRUE)), main = "Sum of intensities\nAfter correction"
            )
            ...
            if (length(norm.ion) > 0) {
                acplight(Xn[, c(indtypsamp, indbatch, indtypsamp, indfact, norm.ion)], "uv", TRUE)
            }
    ...
    dimnames(Xr)[[1]] <- Xn[[1]]
    Xr <- t(Xr)
    Xr <- data.frame(ions = rownames(Xr), Xr)

    res.norm[[1]] <- Xr
    res.norm[[2]] <- data.frame(metaion, res.ind)
    res.norm[[3]] <- x[, c(1:nbid)]
...
acplight <- function(ids, scaling = "uv", indiv = FALSE, indcol = NULL) {
    ...
    for (i in 1:3) {
        ...
        if (i == 1) {
            if (ncol(ions) != (ncol(idss) - 4)) {
                cat("Note:", (ncol(idss) - 4) - ncol(ions), "ions were ignored for PCA display due to NA in intensities.\n")
            }
        }
        # Scaling choice: "uv","none","pareto"
        object <- suppressWarnings(prep(ions, scale = scaling, center = TRUE))
        if (i == 1) {
            if (length(which(apply(ions, 2, var) == 0)) > 0) {
                cat("Warning: there are", length(which(apply(ions, 2, var) == 0)), "constant ions.\n")
            }
        }
    ...
    if (indiv) {
        colour <- rep("darkblue", length(resulti@loadings))
        if (!is.null(indcol)) {
            colour[-c(indcol)] <- "red"
        }
        plot(resulti@loadings,
            col = colour, main = "Loadings", xaxt = "n", yaxt = "n", pch = 20,
